...
router.get("/",authenticate, async (req, res) => {
  try {
    ...
    res.render("scholarship.ejs", { studentNames, data, title: "Scholarship list", jsFile: "scholarship", cssFile: "studentsList" });
  } catch (error) {
...
router.get("/filter",authenticate, async (req, res) => {
  ...
  try {
    ...
    res.render("scholarship.ejs", { studentNames, data, title: "Scholarship list - filter" , jsFile: "scholarship", cssFile: "studentsList"});
  } catch (error) {
    return res.status(500).json({ error: "Server error" });
  }
});
